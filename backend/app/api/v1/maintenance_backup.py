...
from app.schemas.maintenance import MaintenanceBase, MaintenanceResponse, MaintenanceListResponse
...
async def get_maintenances(
    ...
    """
    query = select(Maintenance, Vehicle).outerjoin(Vehicle, Maintenance.vehicle_id == Vehicle.id)
    if vehicle_id:
...
async def get_maintenance(
    ...
    """
    query = select(Maintenance, Vehicle).outerjoin(Vehicle, Maintenance.vehicle_id == Vehicle.id).where(Maintenance.id == maintenance_id)
    result = await db.execute(query)
    row = result.first()
