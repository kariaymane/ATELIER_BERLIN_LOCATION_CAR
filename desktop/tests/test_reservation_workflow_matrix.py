"""
MAXIMUM regression TEST MATRIX — reservation availability & client integrity.
...
def env(qapp):
    ...
    session.merge(LocalVehicle(
        id="fx-veh", registration="regression-1-1", vin="1M8GDM9AXKP042788",
        brand="regressionBrand", model="ProofModel", year=2026, color="Blanc",
...
def _vehicle_dict():
    return {"id": "fx-veh", "brand": "regressionBrand", "model": "ProofModel",
            "registration": "regression-1-1", "daily_rental_price": 250.0}
...
# ─────────────── exact acceptance case (regressionBrand ProofModel) ───────────────

def test_exact_acceptance_case_24_25_august(qapp, env):
    """THE #1 acceptance test: regressionBrand ProofModel 24/08 22:12 -> 25/08 22:12."""
    ...
    dlg._customer_name.setText("regression Client")
...
def test_different_vehicle_not_blocked(qapp, env):
    ...
    widget = _widget()
    created, _ = _create(widget, {
        "vehicle_id": "fx-veh", "customer_name": "A", "customer_phone": "+12025550102",
        "start_datetime": _iso(5), "end_datetime": _iso(6)})
    assert created
    created2, _ = _create(widget, {
        "vehicle_id": "fx-veh2", "customer_name": "B", "customer_phone": "+12025550103",
        "start_datetime": _iso(5), "end_datetime": _iso(6)})
...
def test_completed_reservation_does_not_block(qapp, env):
    ...
    s.commit(); s.close()
    created, _ = _create(widget, {
        "vehicle_id": "fx-veh", "customer_name": "X", "customer_phone": "+12025550104",
        "start_datetime": _iso(10), "end_datetime": _iso(11)})
...
def test_technical_errors_never_report_conflict(qapp, env, http_error, expected_key):
    ...
    created, warnings = _create(widget, {
        "vehicle_id": "fx-veh", "customer_name": "T", "customer_phone": "+12025550107",
...
def test_active_maintenance_blocks(qapp, env):
    ...
    widget = _widget()
    created, warnings = _create(widget, {
        "vehicle_id": "fx-veh", "customer_name": "M", "customer_phone": "+12025550108",
...
def test_completed_maintenance_does_not_block(qapp, env):
    ...
    widget = _widget()
    created, _ = _create(widget, {
        "vehicle_id": "fx-veh", "customer_name": "M", "customer_phone": "+12025550109",
        "start_datetime": _iso(26), "end_datetime": _iso(27)})
...
def test_cancelled_maintenance_does_not_block(qapp, env):
    ...
    widget = _widget()
    created, _ = _create(widget, {
        "vehicle_id": "fx-veh", "customer_name": "M", "customer_phone": "+12025550110",
        "start_datetime": _iso(26), "end_datetime": _iso(27)})
