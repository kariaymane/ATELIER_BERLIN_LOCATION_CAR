"""
Client-selection / write-back regression — New Reservation form.
...
def env(qapp):
    ...
    session.merge(LocalClient(
        id="cw-cli-a", first_name="CustomerC", last_name="ExampleB",
        phone="+12025550101", email="customerc@example.test", cin_number="TEST-CIN-001",
        status="ACTIVE", created_at=now, updated_at=now, version=1))
    session.merge(LocalClient(
        id="cw-cli-b", first_name="CustomerE", last_name="ExampleC",
        phone="+12025550102", email="customere@example.test", cin_number="TEST-CIN-003",
        status="ACTIVE", created_at=now, updated_at=now, version=1))
    session.commit()
    session.close()
    yield
...
def test_switching_between_clients_no_stale_data(qapp, env):
    ...
    assert "CustomerC" in dlg._customer_name.text()
    assert dlg._customer_phone.text() == "+12025550101"
    assert dlg._customer_cin.text() == "TEST-CIN-001"

    dlg._client_combo.setCurrentIndex(_index_for_client(dlg, "cw-cli-b"))
    assert "CustomerE" in dlg._customer_name.text()
    assert "CustomerC" not in dlg._customer_name.text()
    assert dlg._customer_phone.text() == "+12025550102"
    assert dlg._customer_cin.text() == "TEST-CIN-003"
    dlg.close()
...
def test_editing_existing_client_writes_back_to_client_record(qapp, env):
    ...
    dlg = ReservationFormDialog(_vehicle_dict(), api_client=None)
    dlg._client_combo.setCurrentIndex(_index_for_client(dlg, "cw-cli-a"))
    dlg._customer_phone.setText("+12025550123")  # edited
    ...
    assert client.phone == "+12025550123", "edited phone must write back to the Client record"
    assert client.first_name == "CustomerC", "untouched fields must survive the write-back"
...
def test_reopen_shows_persisted_updated_data_not_stale(qapp, env):
    ...
    session = get_local_session()
    session.query(LocalClient).filter_by(id="cw-cli-a").update({"phone": "+12025550120"})
    session.commit()
    session.close()

    dlg = ReservationFormDialog(_vehicle_dict(), api_client=None)
    dlg._client_combo.setCurrentIndex(_index_for_client(dlg, "cw-cli-a"))
    assert dlg._customer_phone.text() == "+12025550120"
    dlg.close()
