...
def test_apply_pulled_client_create(setup_db):
    ...
    assert c.first_name == "CustomerJ"
    assert c.last_name == "ExampleD"
    assert c.version == 2

def test_apply_pulled_client_update(setup_db):
    session = setup_db
    client = LocalClient(id="c2", first_name="CustomerG", last_name="ExampleI", version=1)
    session.add(client)
    session.commit()
    engine = SyncEngine(device_id="test-device")
    item = {
        ...
        "operation": "UPDATE",
        "payload": {
            "first_name": "CustomerH",
            "last_name": "ExampleI",
            "phone": "987654321",
            "email": "customerh@example.com",
            "notes": "updated",
        },
        "version": 3,
    }
    ...
    assert c.first_name == "CustomerH"
    assert c.phone == "987654321"
    assert c.version == 3

def test_apply_pulled_client_delete(setup_db):
    session = setup_db
    client = LocalClient(id="c3", first_name="CustomerF")
    session.add(client)
    session.commit()
    engine = SyncEngine(device_id="test-device")
    item = {
