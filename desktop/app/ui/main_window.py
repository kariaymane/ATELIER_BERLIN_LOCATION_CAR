...
class MainWindow(QMainWindow):
    ...
    def _refresh_dashboard(self):
        ...
        try:
            ...
            today_res = [r for r in all_res if parse_dt(r.start_datetime) and parse_dt(r.start_datetime) >= today_start]
            week_res = [r for r in all_res if parse_dt(r.start_datetime) and parse_dt(r.start_datetime) >= start_of_week]
            month_res = [r for r in all_res if parse_dt(r.start_datetime) and parse_dt(r.start_datetime) >= start_of_month]
            ...
            today_res_c = [r for r in all_res_count if parse_dt(r.start_datetime) and parse_dt(r.start_datetime) >= today_start]
            week_res_c = [r for r in all_res_count if parse_dt(r.start_datetime) and parse_dt(r.start_datetime) >= start_of_week]
            month_res_c = [r for r in all_res_count if parse_dt(r.start_datetime) and parse_dt(r.start_datetime) >= start_of_month]
    ...
    def _update_vehicle(self, data: dict):
        ...
        try:
            ...
            session.rollback()
            QMessageBox.critical(self, t("common.error"), f"Erreur lors de la modification: {e}")
        except Exception as e:
            print("ERROR IN LOAD:", e)
        finally:
            session.close()
    ...
    def _on_delete_vehicle(self, vehicle_id: str):
        ...
        try:
            ...
            session.rollback()
            QMessageBox.critical(self, t("common.error"), f"Erreur lors de la suppression: {e}")
        except Exception as e:
            print("ERROR IN LOAD:", e)
        finally:
            session.close()
